    ...
    for target_materi_idx in range(1, 7):
        ...
        for i in range(n_per_class):
            ...
            for m in range(1, 7):
                s_idx = (m - 1) * 3
                correct_count = sum(
                    [1 for k in range(s_idx, s_idx+3) if student_answers[k] == answer_key[k]])
            ...
            min_indices = [idx+1 for idx,
                           v in enumerate(score_values) if v == min_val]

            # Prioritaskan target kita jika ada di minimum (untuk menjaga label tetap konsisten)
            final_rec = material_map[target_materi_idx] if target_materi_idx in min_indices else material_map[min_indices[0]]
