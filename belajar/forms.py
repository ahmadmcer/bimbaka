...
class KuisForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'deskripsi': forms.Textarea(attrs={'class': 'w-full p-2 border rounded', 'rows': 3}),
            'kelas_target': forms.Select(attrs={'class': 'w-full p-2 border rounded'}),
            'durasi_menit': forms.NumberInput(attrs={'class': 'w-full p-2 border rounded'}),
...
class SoalKuisForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'jawaban_benar': forms.Select(attrs={'class': 'w-full p-2 border rounded bg-green-50'}),
        }